...
def create_account():
    ...
    if form.validate_on_submit():
        ...
        new_transaction = Transaction(
            'deposit', 'account opening', new_account.id, balance)
...
@app.route('/json/account/transactions/<user>')
def get_transaction(user):
    account = Account.query.filter_by(name=user).first()
    transactions = Transaction.query.filter_by(
        account_id=account.id).order_by(Transaction.date.desc()).all()

    #data=dict.fromkeys([x for x in range(len(transactions))],transactions)
    return jsonify(data=str(transactions))
...
def my_account():
    ...
    if deposit_form.deposit.data and deposit_form.validate():
        ...
        if account.deposit_withdraw('deposit', amount):
            new_transaction = Transaction(
                'deposit', 'self deposit', account.id, amount)
            db.session.add(new_transaction)
            db.session.commit()
            return redirect(url_for('my_account'))
        else:
        ...
        if account.deposit_withdraw('withdraw', amount):
            new_transaction = Transaction(
                'withdraw', 'self withdraw', account.id, (amount*(-1)))
            db.session.add(new_transaction)
            db.session.commit()
            return redirect(url_for('my_account'))
        else:
        ...
        if check_password_hash(account.password, password):
            if account.deposit_withdraw('withdraw', amount):
                new_transaction = Transaction(
                    'transfer out', f'transfer to account {account_id}', account.id, (amount*(-1)))
                db.session.add(new_transaction)
                recipient = Account.query.get(account_id)
                if recipient.deposit_withdraw('deposit', amount):
                    new_transaction2 = Transaction(
                        'transfer in', f'transfer from account {account.id}', account_id, amount)
